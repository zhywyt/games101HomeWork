...
Vector3f Scene::castRay(const Ray &ray, int depth) const
{
	...
	{
	    float pdf = intersToScene.m->pdf(ray.direction, wi, intersToScene.normal);
	    L_indir = castRay(indirRay, depth + 1) * intersToScene.m->eval(ray.direction, wi, intersToScene.normal) * dotProduct(wi, intersToScene.normal) / RR / pdf;
	}
